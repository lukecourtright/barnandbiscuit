...
- **Site footer** (`.site-footer`) — a small centered `support@rinkcollective.com` mailto link + copyright line at the very bottom of the page, below the footer CTA banner. Same markup duplicated in `guides.html` (not shared, usual convention). Deliberately just the email for now — no Terms of Use/Privacy Policy links yet, since those need a real decision on corp structure first (see Not Yet Implemented below); the signup consent checkbox's Terms/Privacy links are a separate, still-unrelated `#` placeholder (see Account & Onboarding above). `index.html`/`equipment.html` don't have this footer — both are fixed-viewport (`overflow: hidden`) app shells with no scrolling document body for a footer to sit at the bottom of, unlike this page and Guides.
