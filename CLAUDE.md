...
`AVANTLINK_AFFILIATE_ID`, `AVANTLINK_WEBSITE_ID` — used by `scripts/fetch_avantlink_products.py` to pull live Equipment product/price data from AvantLink (Pure Hockey's affiliate network — see Equipment section below). Requires an approved AvantLink affiliate account and an approved relationship with the target merchant. Not required locally or in prod for the app to run — the script is a standalone tool, not called by `main.py` at runtime; without these set, Equipment just keeps serving `equipment.json`'s mock pricing/retailers as it does today. This is the active pilot network as of this writing; see Equipment section for why Amazon PA-API (`AMAZON_PA_API_ACCESS_KEY`/`AMAZON_PA_API_SECRET_KEY`/`AMAZON_PA_API_PARTNER_TAG`, used by `scripts/fetch_amazon_products.py`) is on hold.
...
│   ├── fetch_avantlink_products.py # AvantLink pipeline for live Equipment offers (search/apply/refresh) — active pilot, see Equipment section below
...
- **Buy/View links are inert placeholders** (`href="#"`, `app.noop()` calls `preventDefault()`/`stopPropagation()`) — still true even for products with live Amazon offers (see Live Offers below); wiring real navigation into the frontend is a separate, not-yet-done step. See Not Yet Implemented below.

### Live Offers (`EquipmentOffer`, `EquipmentPriceSnapshot`, `scripts/fetch_avantlink_products.py`)

The path from mock catalog to a real, purchasable one: `equipment.json` stays the source of truth for *curated* fields (`category`, `brand`, `name`, `specs`, `imageUrl`, `featuredQuote`, `reviewList`, `rating`, `reviewCount`) exactly as today, but pricing/retailer data can now come from a live source instead. Two new tables, populated only by the fetch scripts below and never touched by `sync_equipment_from_file()`:
- **`EquipmentOffer`** — one row per (product, retailer) live listing: `equipmentId`, `retailerName`, `network` (e.g. `"avantlink"`, `"amazon-pa-api"`), `sourceProductId` (SKU/ASIN), `sourceMerchantId` (network-assigned merchant/advertiser id, needed to re-look-up a specific offer directly rather than re-searching by keyword — not every network needs this, so it's nullable), `price`, `url` (affiliate-tagged), `inStock`, `lastCheckedAt`.
...
**AvantLink is the active pilot network** — chosen over Amazon PA-API because Amazon requires 10 sales in 30 days before granting PA-API access (a chicken-and-egg gate: you need the API to build the catalog that drives those sales), while AvantLink's `ProductSearch`/`ProductPriceCheck` APIs have no such sales-volume gate, just a standard affiliate-application review. Pure Hockey — already referenced in `equipment.json`'s mock data — runs its affiliate program on AvantLink. `scripts/fetch_avantlink_products.py` mirrors the `match`/`apply` two-phase pattern of `scripts/fetch_google_places_data.py`:
1. `search --category Sticks [--merchant-id ID] [--limit N]` — queries AvantLink's `ProductSearch` API for a category (optionally scoped to one merchant, e.g. Pure Hockey), writes SKU candidates to a review CSV. No writes yet.
2. Hand-review that CSV (blank out bad matches).
3. `apply <csv>` — for each remaining SKU, if it already has an `EquipmentOffer`, updates its price/url in place, otherwise appends a brand-new curated entry to `equipment.json` and creates its first `EquipmentOffer`/`EquipmentPriceSnapshot`. AvantLink's `Buy_URL` is already a live, ready-to-use affiliate tracking link — no manual tag construction needed (unlike Amazon).
4. `refresh` — calls `ProductPriceCheck` (a direct `merchant_id`+`sku` lookup, not a keyword search) for every existing AvantLink `EquipmentOffer` and logs a new `EquipmentPriceSnapshot`. No CSV/review step. Intended to run on a recurring schedule (daily) — no scheduling infrastructure exists yet (see Not Yet Implemented), so for now this is a manual/cron-to-be-added step. Unlike Amazon's PA-API, AvantLink's generous rate limit (3,600/hr, 15,000/day) and lack of a sales gate mean this is safe to run from day one.

Requires `AVANTLINK_AFFILIATE_ID`/`AVANTLINK_WEBSITE_ID` (see Env vars above) and an approved AvantLink affiliate account with an approved relationship to the target merchant — not yet obtained as of this writing, so the script is scaffolded but unrun; its AvantLink field/param names are written from AvantLink's documented API shape and should be spot-checked against a real response the first time it's run, particularly `ProductPriceCheck`'s exact parameter names (inferred from `ProductSearch`'s conventions, not directly confirmed).

`scripts/fetch_amazon_products.py` — same shape, built first, now **on hold** pending Amazon's PA-API sales-volume gate (see above). Kept as-is since it's still valid once that clears; requires `AMAZON_PA_API_ACCESS_KEY`/`AMAZON_PA_API_SECRET_KEY`/`AMAZON_PA_API_PARTNER_TAG`.
...
- Real affiliate program for Equipment — application to AvantLink (Pure Hockey's network, the active pilot after Amazon PA-API turned out to require 10 sales in 30 days before granting API access — see Live Offers above) is in progress, and the backend/data pipeline for it is scaffolded (`EquipmentOffer`/`EquipmentPriceSnapshot` tables, `scripts/fetch_avantlink_products.py`) but unrun pending credentials. Even once real offers exist, every "Buy"/"View →" link in `static/equipment.html` is still an inert placeholder (`href="#"`, `app.noop()`) — the frontend hasn't been switched over to use real offer URLs yet, and the affiliate disclosure copy in the detail drawer is still placeholder text pending legal review. Other retailers/networks (Amazon once its sales gate clears, HockeyMonkey via Rakuten/Pepperjam, Ice Warehouse via Awin) aren't planned until after the AvantLink pilot proves out, and matching the same physical product across multiple retailers/networks has no reliable automated path — it'll need the same hand-reviewed CSV matching approach these scripts already use.
